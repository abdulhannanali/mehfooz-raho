...
export default function CentresDisplay() {
  ...
  const query = useAppSelector(state => queriesSelectors.selectById(
    ...
    finalElement = (
      <React.Fragment>
        ...
        <Row justify="space-around">
          <Col>
            <PaginationComponent response={centresResponse} onChange={onChangePage}/>
          </Col>
        </Row>
      </React.Fragment>
    )
  ...
  return (
    <React.Fragment>
      <CentresFilter currentFilter={initialFilter} onChangeFilter={onChangeFilter}/>
      {finalElement}
    </React.Fragment>
  );
}
...
function useUpdatedFilterLocation() {
  ...
  return (filter: VaccinationCentresFilter) => {
      const newPath = `${history.location.pathname}${serializeFilterToParams(filter)}`
